...
//
// The remaining devices form the allocator's immutable seed set of already-allocated in-cluster devices.
func (p *Provisioner) gatherAllocatedDevices(ctx context.Context, deletingPodUIDs sets.Set[types.UID]) (sets.Set[cloudprovider.DeviceID], error) {
	seq, err := p.deviceAllocationController.AllocatedDevices(ctx)
	if err != nil {
		return nil, fmt.Errorf("getting allocated devices, %w", err)
	}
