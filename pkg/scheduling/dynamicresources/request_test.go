...
var _ = Describe("Request Validation", func() {
	...
	Describe("ValidateClaimRequest", func() {
		It("should validate a simple ExactCount request", func() {
			...
			data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, pools, nil, celCache, nil)
		...
		It("should fail when DeviceClass is not found", func() {
			...
			_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, pools, nil, celCache, nil)
		...
		It("should fail when an All mode request encounters an invalid pool", func() {
			...
			_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, invalidPools, nil, celCache, nil)
			Expect(err).To(HaveOccurred())
		...
		It("should fail when an All mode request encounters an incomplete pool", func() {
			...
			_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, incompletePools, nil, celCache, nil)
			Expect(err).To(HaveOccurred())
		...
		It("should pass bindingFallback to MatchAttribute constraints", func() {
			...
			data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, pools, nil, celCache, fallback)
			Expect(err).ToNot(HaveOccurred())
			Expect(data.Constraints).To(HaveLen(1))
			mac, ok := data.Constraints[0].(*dynamicresources.MatchAttributeConstraint)
			Expect(ok).To(BeTrue())
		...
		Context("All mode", func() {
			...
			It("should populate AllTemplateDevicesByIT for All mode with template devices", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, nil, templateDevices, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should filter All mode devices by selectors", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, gpuPools, nil, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should keep instance types with no matching template devices when in-cluster devices exist", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, inClusterPools, templateDevices, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should aggregate devices across multiple pools in All mode", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, multiPools, nil, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should fail when any pool is invalid in All mode even if others are valid", func() {
				...
				_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, mixedPools, nil, celCache, nil)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("invalid"))
			})
			...
			It("should return empty AllDevices when no pool devices match selectors in All mode", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, a100Pools, nil, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should validate All mode with empty pools", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, nil, nil, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
				Expect(data.Requests[0].AllDevices).To(BeEmpty())
			})
			...
			It("should fail when All mode in-cluster devices exceed AllocationResultsMaxSize", func() {
				...
				_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, bigPools, nil, celCache, nil)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("exceeding maximum"))
			})
			...
			It("should prune instance types whose template devices would exceed AllocationResultsMaxSize", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, largePools, templateDevices, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should account for ExactCount devices in base total when pruning template devices", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, mixedPools, templateDevices, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should fail when all instance types are pruned", func() {
				...
				_, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, largePools, templateDevices, celCache, nil)
				Expect(err).To(HaveOccurred())
			...
			It("should not fail when template devices exist but no All mode requests reference them", func() {
				...
				data, err := dynamicresources.ValidateClaimRequest(ctx, env.Client, claim, 0, pools, templateDevices, celCache, nil)
				Expect(err).ToNot(HaveOccurred())
				Expect(data.Requests[0].AllocationMode).To(Equal(resourcev1.DeviceAllocationModeExactCount))
			})
		})
